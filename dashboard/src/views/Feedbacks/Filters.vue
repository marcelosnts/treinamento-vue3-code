  ...
  <div class="flex flex-col">
    ...
    <ul class="flex flex-col mt-3 list-none">
      <li
        ...
        class="flex items-center justify-between px-4 rounded cursor-pointer"
      >
...
function serializeFilters (summary) {
...
export default {
  async setup (props, { emit }) {
    ...
    try {
      ...
      state.filters = serializeFilters(data)
    } catch (error) {
      state.hasError = !!error
      state.filters = serializeFilters({
        all: 0,
        issue: 0,
        idea: 0,
        other: 0
